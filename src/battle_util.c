...
u32 AbilityBattleEffects(enum AbilityEffect caseID, enum BattlerId battler, enum Ability ability, enum Move move, bool32 shouldAbilityTrigger)
{
    ...
    case ABILITYEFFECT_ON_SWITCHIN:
        ...
        case ABILITY_ORBITING:
            if (shouldAbilityTrigger)
            {
                gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_SWITCHIN_ORBITING;
                BattleScriptCall(BattleScript_SwitchInAbilityMsg);
                effect++;
            }
            break;
    ...
    case ABILITYEFFECT_ENDTURN:
        if (IsBattlerAlive(battler))
        {
            ...
            case ABILITY_ICE_BODY:
                ...
                break;
            case ABILITY_FERMENT:
                if (gBattleMons[battler].hp <= GetNonDynamaxMaxHP(battler) / 2
                 && !IsBattlerAtMaxHp(battler)
                 && !gBattleMons[battler].volatiles.healBlock)
                {
                    BattleScriptCall(BattleScript_FermentActivates);
                    SetHealAmount(battler, 20);
                    effect++;
                }
                break;
            case ABILITY_DRY_SKIN:
...
enum Obedience GetAttackerObedienceForAction(void)
{
    ...
    return OBEYS; // Obedience checks disabled: all Pokémon, including outsiders, always obey.
...
static inline uq4_12_t CalcTypeEffectivenessMultiplierInternal(struct DamageContext *ctx, uq4_12_t modifier)
{
    ...
    {
        modifier = UQ_4_12(0.0);
        if (ctx->updateFlags && (ctx->abilities[ctx->battlerDef] == ABILITY_LEVITATE || ctx->abilities[ctx->battlerDef] == ABILITY_NUCLEAR_FUSION || ctx->abilities[ctx->battlerDef] == ABILITY_ORBITING))
        {
...
uq4_12_t CalcPartyMonTypeEffectivenessMultiplier(enum Move move, enum Species speciesDef, enum Ability abilityDef)
{
    ...
    if (move != MOVE_STRUGGLE && moveType != TYPE_MYSTERY)
    {
        ...
        if (ctx.moveType == TYPE_GROUND && (abilityDef == ABILITY_LEVITATE || abilityDef == ABILITY_NUCLEAR_FUSION || abilityDef == ABILITY_ORBITING) && !(gFieldStatuses & STATUS_FIELD_GRAVITY))
            modifier = UQ_4_12(0.0);
