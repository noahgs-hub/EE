...
static void TryDrawIconInSlot(enum Species species, s16 x, s16 y)
{
    if (species == SPECIES_NONE || species > NUM_SPECIES)
        ...
        CreateMonIcon(species, SpriteCB_MonIcon, x, y, 0, 0xFFFFFFFF); //shown even if not seen yet
...
static enum Species DexNavGetSpecies(void)
{
    ...
    // unseen species are selectable/searchable too
    return species;
}
...
u32 CalculateDexNavShinyRolls(void)
{
    ...
    levelBonus = (level >= 100) ? 40 : (level >= 50) ? 5 : 0;
