...
// Depth 5 (4^5 = 1024 expansions). Raised from 4 (256) so the all-time TM set
// (488 TMs) can be zipped against NUMBERS_512 in constants/items.h.
#define RECURSIVELY(...) RECURSIVELY_5(RECURSIVELY_5(RECURSIVELY_5(RECURSIVELY_5(__VA_ARGS__))))
#define RECURSIVELY_5(...) RECURSIVELY_4(RECURSIVELY_4(RECURSIVELY_4(RECURSIVELY_4(__VA_ARGS__))))
